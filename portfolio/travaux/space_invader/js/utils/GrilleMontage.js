...
export class GrilleMontage {
	 ...
	 */
	placerColonneLigne(cX, cY, obj, centre = true) {
		//Si la valeur de centre est à true - valeur par défaut
		...
		//console.log("placer", centre);
		let posX, posY;
		if (centre === true) {
			posX = (this.cw * cX) - this.cw / 2;
			posY = (this.ch * cY) - this.ch / 2;
		} else {
			//console.log("A tester");
			posX = (this.cw * cX) - this.cw;
			posY = (this.ch * cY) - this.ch;
		}
	 ...
	 * @param {object Phaser.Objects}   obj - Objets du jeu à placer
	 */
	placerIndexCellule(index, obj, centre = true) {
		...
		this.placerColonneLigne(cX, cY, obj, centre);
	}
